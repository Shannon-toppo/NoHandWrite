...
- **🖨 組版** — 文章をmm指定(文字サイズ・文字間・行間・行幅・余白、A4/B5の用紙プリセットあり。文字間は負の値で詰め組みも可。字送りはデフォルトで可変幅=各文字の字幅で送り、チェックを外すと従来の等幅。縦書きモードあり=右から左への段組みで、ー・括弧類は90°回転、句読点・小書き仮名は右上寄せ)でページにレイアウトし、プレビューを確認してペンプロッター用G-code(送り速度・ペン上下コマンド・Y軸反転も指定可)/ SVG を出力

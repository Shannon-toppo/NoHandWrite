...
- **🖨 組版** — 文章をmm指定(文字サイズ・文字間・行間・行幅・余白、A4/B5の用紙プリセットあり。文字間は負の値で詰め組みも可。字送りはデフォルトで可変幅=各文字の字幅で送り、チェックを外すと従来の等幅)でページにレイアウトし、プレビューを確認してペンプロッター用G-code(送り速度・ペン上下コマンド・Y軸反転も指定可)/ SVG を出力
